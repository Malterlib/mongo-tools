...
extern int X_SSL_CTX_deduce_settings_from_certificate(SSL_CTX* context) {
    int curveName = 0;

    EVP_PKEY *privateKey = SSL_CTX_get0_privatekey(context);
    if (privateKey) {
    	RSA *keyRSA;
    	EC_KEY *keyEC;
        if ((keyRSA = EVP_PKEY_get1_RSA(privateKey))) {
            unsigned RSASize = RSA_size(keyRSA) * 8;
            RSA_free(keyRSA);
        ...
        } else if ((keyEC = EVP_PKEY_get1_EC_KEY(privateKey))) {
